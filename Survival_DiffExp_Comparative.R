...
rna.common_names <- rna[common_names,]
# Create directory (if it doesn't exist)
if (!dir.exists("Survival_Plot")) dir.create("Survival_Plot")   ### change directory name here
  ...
  for (i in 1:nrow(as.matrix(rownames(dataNormal)))) {
    ...
    if (!is.na(mRNAselected_values_ordered_top)) {
      ...
      if (Survresult == TRUE) {
        ...
        #print(tabSurv)
        file_name = paste0("Survival_Plot/KM_Plot_", mRNAselected, ".pdf")
...
results <- plot_surv(clin, dataGE = rna, Genelist = rownames(rna.common_names), ThreshTop = 0.6, ThreshDown = 0.4, Survresult = TRUE, p.cut = 0.05) # I already defines group1= normal and group2=cancer
#results <- plot_surv(clin, dataGE = rna, Genelist = rownames(results), ThreshTop = 0.6, ThreshDown = 0.4, Survresult = TRUE, p.cut = 0.05)
results1 <- results[(results$`Cancer Deaths with Top` >=3 & results$`Cancer Deaths with Down` >=3) & results$pvalue <= 0.01,]
unlink("Survival_Plot/KM_Plot_*")
results1.DEG <- plot_surv(clin, dataGE = rna, Genelist = rownames(results1), ThreshTop = 0.6, ThreshDown = 0.4, Survresult = TRUE, p.cut = 0.01)
