...
setwd("F:/OneDrive - University of Nebraska Medical Center/eMap/RTCGAToolbox/TCGABiolink/Epigenetic Silencing")
library(pacman)
p_load(multtest, matrixStats, plyr)
#######################################################
load("../GDC-Harmonized/CHOL/CHOL.RData")
...
rm(hm450.anno)
## JHU.USC.annotation.txt generated by using JHU_USC_annotation.R file
## This R program use GDC level3 DNA methylation file "jhu-usc.edu_CHOL.HumanMethylation450.1.lvl-3.TCGA-YR-A95A-01A-12D-A418-05.gdc_hg38.txt"
hm450.anno.epig <-read.table("JHU.USC.annotation.txt",header=T,sep="\t",na.strings="NA",row.names=1,quote="",comment.char="", stringsAsFactors = FALSE, check.names = FALSE)
hm450.anno.epig <- hm450.anno.epig[which(hm450.anno.epig$Position_to_TSS!='.'),]
hm450.anno.epig <- hm450.anno.epig[!grepl("rs", hm450.anno.epig$ProbeID),] ## Remove CpGs with rs in probe name
...
Epigenetic_silenced_Noushmer <- ddply(dat.meth.exp, .(Gene_Symbol), function(x) x[which.max(x$Mean.Diff),])
...
save.image(file = "../GDC-Harmonized/CHOL/EpigeneticSilence_Noushmer_CHOL.RData")