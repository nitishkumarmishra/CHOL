...
if (!dir.exists("Survival_Plot_DEG")) dir.create("Survival_Plot_DEG")   ### change directory name here
  ...
  for (i in 1:nrow(as.matrix(rownames(dataNormal)))) {
    ...
    if (!is.na(mRNAselected_values_ordered_top)) {
      ...
      if (Survresult == TRUE) {
        ...
        #print(tabSurv)
        file_name = paste0("Survival_Plot_DEG/KM_Plot_", mRNAselected, ".pdf")
...
#results <- plot_surv(clin, dataGE = rna, Genelist = rownames(results), Survresult = TRUE, p.cut = 0.01, Median = TRUE, Sdev = 0)
results1 <- results[results$`Cancer Deaths with Top` >=3 & results$`Cancer Deaths with Down` >=3,]
results.DEG <- results[rownames(results)%in%common_names,]
unlink("Survival_Plot_DEG/KM_Plot_*") 
results1.DEG <- plot_surv(clin, dataGE = rna, Genelist = rownames(results.DEG), Survresult = TRUE, p.cut = 0.01, Median = TRUE, Sdev = 0)
unlink("Survival_Plot_DEG/KM_Plot_*") 
results1.DEG <- plot_surv(clin, dataGE = rna, Genelist = rownames(results1.DEG), Survresult = TRUE, p.cut = 0.01, Median = TRUE, Sdev = 0)
...
save.image("Survival_CHOL_Median_Sd.RData")

library(clusterProfiler)
library(org.Hs.eg.db)
detach("package:dplyr", unload=TRUE)
gene <- common_list[rownames(results.DEG),]$common_id

kk <- enrichKEGG(gene         = gene,
                 #organism     = 'hsa',
                 universe = common_list$common_id,
                 pvalueCutoff = 0.05)
