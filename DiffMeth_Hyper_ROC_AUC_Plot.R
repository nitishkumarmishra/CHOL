...
load("Survival_analysis_Promoter_Methylation_Hyper.RData")
######### Making input file ############
ind_Exp <- as.data.frame(BMIQ.Meth.Hyper.TSS)
...
ind_Exp <- ind_Exp[rownames(BMIQ.Meth.Hyper.TSS),]
...
if (!dir.exists("DiffMeth_Hyper_ROC_Plot")) dir.create("DiffMeth_Hyper_ROC_Plot")   ### change directory name here
...
for (i in 1:length(rocs)) {
  ...
  if (auc > 0.5) {
    file_name = paste0("DiffMeth_Hyper_ROC_Plot/roc_", name, ".png")    ### change directory name here (the one you created above.
    png(file_name)
...
write.csv(auc_df, "AUC_table_DiffMeth_Hyper_ROC_Plot.csv", row.names = F)   ### change filename to save AUC values.
save.image("DiffMeth_Hyper_ROC_Plot.RData")


auc_df[auc_df$name%in%rownames(results.DM),]
