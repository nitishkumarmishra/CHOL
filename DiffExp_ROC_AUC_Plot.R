...
load("Survival_CHOL_Mean_Sd.RData")
######### Making input file ############
...
if (!dir.exists("DiffExp_ROC_Plot")) dir.create("DiffExp_ROC_Plot")   ### change directory name here
...
for (i in 1:length(rocs)) {
  ...
  if (auc > 0.5) {
    file_name = paste0("DiffExp_ROC_Plot/roc_", name, ".png")    ### change directory name here (the one you created above.
    png(file_name)
...
write.csv(auc_df, "AUC_table_DiffExp_ROC_Plot.csv", row.names = F)   ### change filename to save AUC values.
save.image("DiffExp_ROC_Plot.RData")
...
setwd("F:/OneDrive - University of Nebraska Medical Center/eMap/RTCGAToolbox/TCGABiolink/dataTGCT/CHOL/")
#direct <- getwd()
#tmp <- unique(c(rownames(diffMiRNA_Surv_Median), rownames(diffMiRNA_Surv_Quantile)))
file_name = paste0("roc_", rownames(DiffExp_Surv_0.05), ".png")
#file_name = paste0("roc_", tmp, ".png")
setwd("DiffExp/DiffExp_ROC_Plot/")
all_file <- list.files(full.names = TRUE, recursive = TRUE)
all_file <- gsub("./", "", all_file)
outersect <- function(x, y) {
  sort(c(setdiff(x, y),
         setdiff(y, x)))
}
outer <- outersect(all_file, file_name)
unlink(outer)

rm(all_file, file_name, tmp, outer)
