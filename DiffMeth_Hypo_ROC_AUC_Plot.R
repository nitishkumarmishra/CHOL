...
load("Survival_analysis_Promoter_Methylation_Hypo.RData")
######### Making input file ############
ind_Exp <- as.data.frame(BMIQ.Meth.Hypo.TSS)
...
ind_Exp <- ind_Exp[rownames(BMIQ.Meth.Hypo.TSS),]
...
if (!dir.exists("DiffMeth_Hypo_ROC_Plot")) dir.create("DiffMeth_Hypo_ROC_Plot")   ### change directory name here
...
for (i in 1:length(rocs)) {
  ...
  if (auc > 0.5) {
    file_name = paste0("DiffMeth_Hypo_ROC_Plot/roc_", name, ".png")    ### change directory name here (the one you created above.
    png(file_name)
...
write.csv(auc_df, "AUC_table_DiffMeth_Hypo_ROC_Plot.csv", row.names = F)   ### change filename to save AUC values.
save.image("DiffMeth_Hypo_ROC_Plot.RData")
