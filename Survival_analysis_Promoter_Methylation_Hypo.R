...
results.dms.hypo.TSS <- results.dms.hypo[rownames(results.dms.hypo)%in%JHU.annotation.TSS$ProbeID,]

rm(symbol, type, transcript, positionTSS, feature)

BMIQ.Meth.Hypo.TSS <- BMIQ.Meth[rownames(results.dms.hypo.TSS),]


clin <- GDCquery_clinic("TCGA-CHOL","clinical")

# Create directory (if it doesn't exist)
if (!dir.exists("Survival_Plot_DM_Hypo")) dir.create("Survival_Plot_DM_Hypo")   ### change directory name here
...
############################################################################
Genelist <- rownames(BMIQ.Meth.Hypo.TSS)
#Genelist1 <- Genelist[1:100]
results <- plot_surv(clin, dataMETH = BMIQ.Meth.Hypo.TSS, Genelist = rownames(BMIQ.Meth.Hypo.TSS), ThreshTop = 0.5, ThreshDown = 0.3, Survresult = TRUE, PercentUp = 20, PercentDown = 20, p.cut = 0.05) # I already defines group1= normal and group2=cancer
unlink("Survival_Plot_DM_Hypo/KM_Plot_*") ## Remove all Survival plot files
results.DM <- plot_surv(clin, dataMETH = BMIQ.Meth.Hypo.TSS, Genelist = rownames(results), ThreshTop = 0.5, ThreshDown = 0.3, Survresult = TRUE, PercentUp = 20, PercentDown = 20, p.cut = 0.05) # I already defines group1= normal and group2=cancer
#results <- results[results$`Cancer Deaths with Top` >4 & results$`Cancer Deaths with Down` >4,]

save.image("Survival_analysis_Promoter_Methylation_Hypo.RData")
