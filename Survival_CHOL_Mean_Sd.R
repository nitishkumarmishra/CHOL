...
if (!dir.exists("Survival_Plot_DEG_Mean")) dir.create("Survival_Plot_DEG_Mean")   ### change directory name here
  ...
  for (i in 1:nrow(as.matrix(rownames(dataNormal)))) {
    ...
    if (!is.na(mRNAselected_values_ordered_top)) {
      ...
      if (Survresult == TRUE) {
        ...
        #print(tabSurv)
        file_name = paste0("Survival_Plot_DEG_Mean/KM_Plot_", mRNAselected, ".pdf")
...
unlink("Survival_Plot_DEG_Mean/KM_Plot_*") 
results1.DEG <- plot_surv(clin, dataGE = rna, Genelist = rownames(results.DEG), Survresult = TRUE, p.cut = 0.01, Mean = TRUE, Sdev = 0)
unlink("Survival_Plot_DEG_Mean/KM_Plot_*") 
...
save.image("Survival_CHOL_Mean_Sd.RData")
